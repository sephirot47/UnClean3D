...
void EffectLayerAmbientOcclusion::AddDirt(GameObject *modelGameObject)
{
    if (!modelGameObject)
    {
        return;
    ...
    for (MeshRenderer *mr : mrs)
    {
        ...
        if (Texture2D *originalAlbedoTex = mat->GetAlbedoTexture())
        {
            ...
            uint triId = 0;
            bool finished = Dialog::LoadBar(
                "Adding dirt...",
                [&](LoadBarInformation *lbi) {
                    ...
                    rMin =
                        Vector2i::Clamp(rMin, Vector2i::Zero(), imgSizei - 1);
                    rMax =
                        Vector2i::Clamp(rMax, Vector2i::Zero(), imgSizei - 1);
                    ...
                    lbi->SetMessage("Adding dirt to triangles...");
                    lbi->SetPercent(SCAST<float>(triId) /
                                    mesh->GetNumTriangles());

                    ++triId;

                    if (triId == mesh->GetNumTriangles())
                    {
                        lbi->Finish();
                    }
                },
